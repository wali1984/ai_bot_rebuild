# Phase 2J.A — Paper-Mode Runtime-Flag Domain Safety Boundaries

This document enumerates the safety boundaries Phase 2J.A MUST honor. Any violation triggers a stop with FAILED gate marker and surfaces to the supervisor.
...
- MUST NOT introduce a `PAPER_MODE_LIVE_ENABLED` constant, a `live_enabled` constant, or any other live-execution affordance.
...
- MUST NOT call wall-clock helpers (`time.time`, `time.monotonic`, `datetime.now`, `datetime.utcnow`) anywhere in the authored source. The 2J.B service layer consumes a `now_ms_clock` callable as a constructor argument; the 2J.A value-object layer never invokes a clock.
- MUST NOT invoke `subprocess`, `os.environ`, or `os.getenv`.
- MUST NOT log via `logging` or stdout.

## Cross-milestone

- MUST NOT modify any prior-milestone source or test file (2E1, 2E2, 2E3, 2F.A, 2F.B, 2F.C, 2G.A, 2G.B, 2G.C, 2H.A, 2H.B, 2H.C, 2I.A, 2I.B, 2I.C).
- MUST NOT modify any 2J.A planning artifact at 00, 01, 02, 03, 04, 05.
...
- MUST NOT add a service-layer assembler (deferred to 2J.B).
- MUST NOT add a composition-root binder (deferred to 2J.C).
- MUST NOT introduce any new lineage ID at the 2J.A value-object layer beyond the typed `PaperModeFlag` itself; the flag is consumed by downstream consumers (`paper_trade_id`, `replay_run_id`, future `shadow_decision_id`) without becoming a new lineage ID itself.
...
- wall-clock helper invocation in any authored 2J.A source file — MUST be "none observed"
...
- introduction of `PAPER_MODE_LIVE_ENABLED`, `live_enabled`, or any live-execution affordance constant — MUST be "none observed"
...
- emission of token `PaperExecutionLedgerEntry` in any authored 2J.A source file — MUST be "none observed"
- emission of token `RiskDecisionRecord` or `OrchestratorDecisionRecord` in any authored 2J.A source file — MUST be "none observed"
- emission of token `ReplayBacktestRun`, `ReplayBacktestStep`, or `ReplayBacktestSummary` in any authored 2J.A source file — MUST be "none observed"
...
PHASE2J_A_PAPER_MODE_RUNTIME_FLAG_DOMAIN_SAFETY_BOUNDARIES_READY
