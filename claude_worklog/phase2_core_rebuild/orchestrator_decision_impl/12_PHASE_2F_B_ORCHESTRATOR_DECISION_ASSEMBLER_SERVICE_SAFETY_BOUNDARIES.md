# Phase 2F.B — Orchestrator Decision Assembler Service Safety Boundaries

This document enumerates hard safety invariants for Phase 2F.B. Codex review at task `120` MUST verify each invariant explicitly and cite evidence for each PASS row.

## Forbidden runtime behaviors (in any authored 2F.B source file)
...
- No `subprocess` invocation outside the four permitted test files (`test_assembler_service_does_not_import_redis.py`, `test_assembler_service_does_not_import_url_env.py`, `test_assembler_service_does_not_register_fastapi_lifespan.py`, and any equivalent fresh-subprocess import isolation test enumerated in `11_PHASE_2F_B_ORCHESTRATOR_DECISION_ASSEMBLER_SERVICE_TEST_PLAN.md`).
- No `socket` import.
...
- No mutation of any 2F.A authored source or test file.
...
The single one-line placeholder file `v2/backend/app/services/orchestrator_decision.py` is deleted by task `119`. After the task completes:

- `git ls-files v2/backend/app/services/orchestrator_decision.py` MUST return zero output lines.
- `git ls-files v2/backend/app/services/orchestrator_decision/__init__.py` MUST return exactly one line.
- `git ls-files v2/backend/app/services/orchestrator_decision/service.py` MUST return exactly one line.
- `git ls-files v2/backend/app/services/orchestrator_decision/errors.py` MUST return exactly one line.

The placeholder file MUST NOT be reintroduced under any condition. Codex review at task `120` re-runs each of the four `git ls-files` checks and confirms the result.
...
The only allowed git-status outputs after task `119` runs are: deletion of `v2/backend/app/services/orchestrator_decision.py`; creation of the new package files; creation of the 37 new test files; creation of `14_2F_B_ORCHESTRATOR_DECISION_ASSEMBLER_SERVICE_IMPLEMENTATION_REPORT.md` and `15_2F_B_ORCHESTRATOR_DECISION_ASSEMBLER_SERVICE_GO_NO_GO.md`.
...
- Any FastAPI lifespan or router or singleton or cache or wall-clock helper or `os.environ` or subprocess (outside the four permitted test files) or socket in any authored 2F.B source file.
- Any direct `redis`, `url_env`, or factory import in any authored 2F.B source file.
- Any URL or credential leakage.
- Any REQ_0017 scope-cap violation (no risk gateway, no execution surface, no FastAPI surface, no adapter expansion, no composition root, no decision-route reasoning beyond the documented derivation table).
- Reintroduction of the placeholder file `v2/backend/app/services/orchestrator_decision.py`.

## Live gate

The live gate remains blocked. The 2F.B service constructs every `OrchestratorDecisionRecord` with `live_blocked=True` so the 2F.A domain invariant trips the construction-time fail-closed if any caller attempts to substitute `False`.

PHASE2F_B_ORCHESTRATOR_DECISION_ASSEMBLER_SERVICE_SAFETY_BOUNDARIES_READY
