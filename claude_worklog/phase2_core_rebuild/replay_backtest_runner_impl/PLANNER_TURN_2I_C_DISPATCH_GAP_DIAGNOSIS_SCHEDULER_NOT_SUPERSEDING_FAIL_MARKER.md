# Planner Turn 2I.C Dispatch Gap Diagnosis Scheduler Not Superseding Fail Marker
...
the already-pending Lane C codex_watchdog reconciliation task. The planner
will re-engage only after the 2I.C Codex GO/NO-GO marker body flips from FAIL
to PASS or the supervisor surfaces a new safety event.

This note is materially distinct from the prior 2I.C stand-down notes because
it adds the new scheduler-fix diagnosis below: the recent scheduler hardening
commits do NOT supersede the 2I.C fail marker. The supervisor dispatch gap
is operational on the supervisor side, not a planner-side decision question.
...
`PLANNER_TURN_2I_C_DISPATCH_AUTHORIZATION_REAFFIRMED_AWAITING_SUPERVISOR_RECONCILIATION_DISPATCH.md`:
...
- `claude_worklog/phase2_core_rebuild/replay_backtest_runner_impl/24_2I_C_REPLAY_BACKTEST_RUNNER_COMPOSITION_ROOT_CODEX_REVIEW.md`
  ...
  placeholder hard stop and identifies the only concrete blocker as the
  row-60 expectation that `git ls-files v2/backend/app/domain/execution/`
  would return zero output lines. The three observed tracked paths are
...
## New Evidence: Scheduler Not Superseding 2I.C Fail Marker

Recent scheduler hardening commits introduced a stage-keyed
fail-marker-supersession filter:

- `6bc936c Stop scheduler advertising superseded fail-marker recovery`
- `fcb82f4 Tighten stale fail marker supersession to one-line Codex pass gates`

The new filter lives in `claude_worklog/tools/parallel_capacity_scheduler.py`
in `marker_stage_key()` (lines 133-141) and `fail_marker_superseded_by_codex_pass()`
(lines 144-165). It computes a stage key from the marker filename via
the regex `2([a-z])[_-]?([a-z])` and treats a fail marker as superseded only
if some other file in the same stage under
`claude_worklog/phase2_core_rebuild/` contains `CODEX_PASS` as a single-line body.

For the 2I.C fail marker:

- Filename
  `25_2I_C_REPLAY_BACKTEST_RUNNER_COMPOSITION_ROOT_CODEX_GO_NO_GO.md`.
- Lowercase name match: `25_2i_c_replay_backtest_runner_composition_root_codex_go_no_go.md`.
- Regex match groups: `i`, `c`. Stage key: `2i_c`.
- 2I.A Codex PASS marker stage key resolves to `2i_a`.
- 2I.B Codex PASS marker stage key resolves to `2i_b`.
- No other file under `claude_worklog/phase2_core_rebuild/` resolves to
  stage key `2i_c` and contains `CODEX_PASS` as a single-line body until
  the recovery task rewrites the 25_ marker body.

Therefore `fail_marker_superseded_by_codex_pass()` returns False for the
2I.C 25_ fail marker today, and `latest_fail_marker()` (lines 168-188) MUST
return the 2I.C 25_ marker path. The recent scheduler hardening commits
do NOT block dispatch of the existing recovery task.

The dispatch gap is not caused by the scheduler-supersession filter. The
gap is operational on the supervisor side. Possible operational causes,
listed for the supervisor watchdog to triage but not for the planner to
fix this turn:

- The supervisor task-queue scanner has not yet picked up the
  `codex_recover_fail_marker_2ic_replay_backtest_runner_composition_root_codex_go_no_go.json`
  task definition since its emission at 02:00.
- The supervisor's worktree-isolation guard is rejecting dispatch despite
  the three documented `worktree_excluded_paths` entries.
- A separate quota or capacity guard on the Codex parallel lane is holding
  dispatch.

The planner does not modify supervisor or scheduler code this turn. The
supervisor watchdog under REQ_0016 / REQ_0021 is the correct authority for
that diagnosis and any safe non-live fix.
...
- No re-emission of the recovery task body or any prior planner-turn note.
- No 2J PAPER_MODE_MVP sub-phase breakdown, legacy-evidence review, or
  domain spec. Those will be opened only after the 25_ marker flips to
...
- No modification of `claude_worklog/tools/parallel_capacity_scheduler.py`
  or any other supervisor/watchdog tooling.
- No modification of the master planner prompt (the existing dirty edit at
  `claude_worklog/autonomous_control_plane/claude_master_rebuild_planner_prompt.txt`
  is the planner-prompt milestone update from the prior turn that the
  recovery task records as a worktree-excluded path).

## Lane / MVP Relevance

- Active lane: `codex_watchdog`.
- Active MVP milestone (REQ_0017): `REPLAY_BACKTEST_RUNNER_MVP` (milestone 5).
- Distance to `V2_BACKTEST_AND_PAPER_MVP_READY`: 3 milestones remaining;
  drops to 2 once the 25_ marker flips to PASS.
...
- Legacy evidence consulted: the 2I.A and 2I.B GO/NO-GO and Codex GO/NO-GO
  ...
  `v2/backend/app/domain/execution/__init__.py`,
  `v2/backend/app/domain/execution/intent.py`, and
  `v2/backend/app/domain/execution/paper.py`; the recent scheduler
  hardening commits `6bc936c` and `fcb82f4` and the relevant scheduler
  source at `claude_worklog/tools/parallel_capacity_scheduler.py:133-188`.
- Legacy behavior preserved: read-only adjudication of pre-existing 015A
  ...
  no mutation of any V2 source or test file; no mutation of any 2H or
  earlier milestone artifact; no mutation of any supervisor/watchdog/
  scheduler tooling.
- Legacy failure addressed: the legacy automation loop required manual
  human intervention to reconcile a CODEX FAIL marker when the only
  observed failure was a pre-existing scaffold placeholder cross-isolation
  conflict that the milestone itself forbids mutating, and additionally
  required manual diagnosis of whether recent scheduler hardening
  inadvertently superseded the 2I.C fail marker. The 2H precedents
  established the watchdog reconciliation pattern; this stand-down note
  reaffirms that the pattern is the correct dispatch path for 2I.C, that
  no planner-side work is required to apply it, and that the scheduler
  fix does not block dispatch of the existing recovery task.
- V2 proof gate: the supervisor's dispatch of the existing Lane C
  ...
  `PHASE2I_C_REPLAY_BACKTEST_RUNNER_COMPOSITION_ROOT_CODEX_PASS` and emit
  a new 26_ reconciliation addendum citing the 015A commit, the zero-byte
  2I.C diff against `v2/backend/app/domain/execution/`, the per-row PASS
  evidence already recorded in 24_ before the placeholder hard stop, and
  the validation re-run from 22_ and 23_.
...
- Any modification of any 2H.A, 2H.B, 2H.C, 2I.A, 2I.B, or 2I.C artifact
  other than the single 25_ marker rewrite, the new 26_ reconciliation
  addendum, and the two `automation_reliability/` report files emitted
  by the recovery task.
- A `CODEX_FAIL_MARKER_RECOVERY_BLOCKED` result from the recovery task
  with a specific failed verification check.
