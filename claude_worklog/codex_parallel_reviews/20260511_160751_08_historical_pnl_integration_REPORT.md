...
The non-live Phase 2 fixture path is integrated and has passing Codex markers, but the historical PnL / trade audit is not quantitatively ready. The current 30-day audit remains partial/local-only: no Binance read-only pull was requested, income/trade/order row counts are zero, PnL by day and symbol are `NO_DATA`, fee/funding/commission drag is `NO_DATA`, and realized trades are not joined to trainer or decision lineage.
...
- prior historical-PnL Codex parallel-review reports for continuity
...
PARTIAL PASS. The LAB hedge-unwind / short-squeeze failure is captured in `legacy_failure_cases/2026_05_LAB_HEDGE_UNWIND_SHORT_SQUEEZE_FAILURE.md`: a protective long was closed around breakeven, residual short exposure remained, and LAB moved approximately 80% against the remaining short.

Phase 2M integrates the case as a non-live typed replay fixture with five outcome variants: legacy action, keep hedge, close short, reduce short, and block hedge close. `claude_worklog/phase2_core_rebuild/replay_case_lab_hedge_unwind/09_CODEX_GO_NO_GO.md` contains `PHASE2M_REPLAY_CASE_LAB_HEDGE_UNWIND_CODEX_PASS`.

The limitation is material and explicitly documented: `02_REPLAY_CASE_OUTCOME_MATRIX.md` says current typed surfaces do not model hedge state, residual exposure, position size, PnL, slippage, fees, funding, OI, liquidation map, orderbook depth, or squeeze risk. It also says close-short and reduce-short collapse to the same typed mirror sequence because partial-close size is not typed yet.
