...
The repository has passing non-live typed evidence harnesses, but it is not ready as a historical PnL / trade audit integration. The 30-day audit remains partial/local-only: no Binance read-only pull was requested, account-history row counts are zero, PnL by day and symbol are `NO_DATA`, and fee/funding/commission drag is `NO_DATA`.
