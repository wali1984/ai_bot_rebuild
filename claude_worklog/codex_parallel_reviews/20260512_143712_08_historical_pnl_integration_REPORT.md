...
The non-live Phase 2 integration scaffolding is present and has passing markers for the LAB replay case, historical-PnL replay wiring, and V2 backtest/paper MVP readiness. The historical PnL audit itself is not evidence-ready: the 30-day account-history pull was not performed, income/trade/order row counts are zero, PnL by day and symbol are `NO_DATA`, fee/funding/commission drag is `NO_DATA`, and realized outcomes are not joined to trainer, decision, risk, feature, ledger, or replay lineage.
...
- `claude_worklog/phase2_core_rebuild/automation_reliability/codex_recover_163_*`
- `claude_worklog/phase2_core_rebuild/automation_reliability/codex_recover_164_*`
...
Phase 2M integrates that failure as a non-live typed replay fixture with five variants: legacy action, keep hedge, close short, reduce short, and block hedge close. `claude_worklog/phase2_core_rebuild/replay_case_lab_hedge_unwind/09_CODEX_GO_NO_GO.md` contains `PHASE2M_REPLAY_CASE_LAB_HEDGE_UNWIND_CODEX_PASS`, and the recovery validation recorded 15 passing tests.

The limitation remains material and explicitly documented: current typed surfaces do not model hedge state, residual exposure, position size, PnL, slippage, fees, funding, OI, liquidation map, orderbook depth, or squeeze risk. Close-short and reduce-short collapse to the same typed mirror sequence because partial-close size is not typed yet.
...
`09_V2_BUILD_IMPACT_MAP.md` maps realized PnL by symbol, fee/funding/commission drag, large winners/losers, trainer/orchestrator evidence, and LAB hedge unwind into the `paper_backtest_mvp` lane. Phase 2P provides deterministic non-live replay wiring and has `PHASE2P_HISTORICAL_PNL_REPLAY_WIRING_CODEX_PASS`, but it is fixture/evidence-pointer integration and explicitly does not perform the live Binance read-only pull, PnL aggregation, fee/funding accounting, or residual-exposure modeling.
...
- The inspected Phase 2M/2P work is non-live, deterministic, and test-only, with inert evidence pointers/counts and no live/exchange/Redis behavior.
