...
The remaining gap is behavioral depth. The Phase 2M review explicitly notes no PnL, sizing, price, fees, slippage, funding, OI, liquidation-map, orderbook-depth, hedge-state, residual-exposure, or squeeze-risk computation. The Phase 2P fixture spec explicitly forbids those fields, so the current implementation cannot yet answer the historical audit questions it points at.

### V2 Risk/Backtest Requirements From Evidence

The documented requirements are directionally correct. `08_FAILURE_PATTERNS_AND_V2_REQUIREMENTS.md` calls for repeated realized-loss detection, fee/funding drag identification, large-loser comparison to trainer confidence and feature freshness, hedge-unwind residual exposure detection, top/bottom short/long detection, stale/missing-data default-deny, and replay/backtest scenarios.

`09_V2_BUILD_IMPACT_MAP.md` maps realized PnL by symbol, fee/funding/commission drag, large winners/losers, trainer/orchestrator evidence, and LAB hedge unwind into the paper/backtest MVP lane.
...
- `V2_BACKTEST_AND_PAPER_MVP_READY_CODEX_PASS` is present for the typed-surface MVP consolidation, while that review also states no PnL computation, sizing, prices, fees, slippage, or ledger persistence were introduced.
