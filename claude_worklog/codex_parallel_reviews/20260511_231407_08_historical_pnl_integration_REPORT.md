...
The non-live Phase 2 integration path is present and has passing Codex markers for LAB replay, historical-PnL replay wiring, aggregate evidence roll-up, and V2 backtest/paper MVP readiness. The historical PnL / trade audit is still not quantitatively ready: the current 30-day audit is partial/local-only, Binance read-only pull was not requested, income/trade/order row counts are zero, PnL by day and symbol are `NO_DATA`, fee/funding/commission drag is `NO_DATA`, and realized outcomes are not joined to trainer/decision/risk lineage.
...
Phase 2M integrates the failure as a non-live typed replay fixture with five variants: legacy action, keep hedge, close short, reduce short, and block hedge close. `claude_worklog/phase2_core_rebuild/replay_case_lab_hedge_unwind/09_CODEX_GO_NO_GO.md` contains `PHASE2M_REPLAY_CASE_LAB_HEDGE_UNWIND_CODEX_PASS`.

The limitation is material and explicitly documented in `02_REPLAY_CASE_OUTCOME_MATRIX.md`: current typed surfaces do not model hedge state, residual exposure, position size, PnL, slippage, fees, funding, OI, liquidation map, orderbook depth, or squeeze risk. Close-short and reduce-short also collapse to the same typed mirror sequence because partial-close size is not typed yet.
