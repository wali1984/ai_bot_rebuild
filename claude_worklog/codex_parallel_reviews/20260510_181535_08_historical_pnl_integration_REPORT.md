...
The repository is safe as non-live typed evidence plumbing, but it is not ready as historical PnL / trade audit integration. The 30-day audit is still partial/local-only: no Binance read-only pull was requested, account-history row counts are zero, PnL by day and symbol are `NO_DATA`, and fee/funding/commission drag is `NO_DATA`.
