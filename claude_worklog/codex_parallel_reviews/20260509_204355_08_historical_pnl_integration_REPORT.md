...
The current work is safe and useful as non-live typed evidence plumbing, but it is not ready as historical PnL / trade audit integration. The 30-day audit remains partial/local-only: no Binance read-only pull was requested, income/trade/order rows are zero, PnL by day and symbol are `NO_DATA`, and fees/funding/commission are `NO_DATA`.
...
`01_DATA_SOURCE_STATUS.md` records `requested_days: 30`, `binance_pull_requested: False`, no Binance API key/secret env names present, and no symbols requested for trade/order history.

`02_BINANCE_READONLY_PULL_SUMMARY.md` records `income_rows: 0`, `trade_rows: 0`, `order_rows: 0`, and `BINANCE_PULL_NOT_REQUESTED`.
...
The legacy failure case is captured: the protective long was closed around breakeven, residual short exposure remained, and LAB then pumped approximately 80% against the remaining short.

Phase 2M has a passing replay-case fixture for legacy, keep-hedge, close-short, reduce-short, and block-hedge-close variants. Phase 2P has a passing historical-PnL replay wiring fixture with a `LABUSDT` loser-short scenario and deterministic `legacy_realized_trade_evidence__lab_hedge_unwind_squeeze__step_N` pointers.

The remaining gap is behavioral depth. The Phase 2M/2P artifacts explicitly do not model PnL, quantity, price, fees, funding, hedge state, residual exposure, liquidation/OI/orderbook context, or squeeze risk. Phase 2P also has tests asserting these fields are absent.

### V2 Risk/Backtest Requirements From Evidence

The documented requirements are directionally correct. The audit packet maps realized PnL by symbol, fee/funding/commission drag, large winners/losers, trainer/orchestrator evidence, and LAB hedge unwind into the paper/backtest MVP lane.

The implementation does not yet satisfy those requirements with real or sanitized account-history evidence. Current Phase 2P/2Q work is deterministic pointer/count plumbing, not populated trade-outcome integration.
