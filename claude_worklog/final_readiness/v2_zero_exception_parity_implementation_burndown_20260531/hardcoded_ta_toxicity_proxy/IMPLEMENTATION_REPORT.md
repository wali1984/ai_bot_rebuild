# Implementation Report — hardcoded_ta_toxicity_proxy
...
## Claim
TA feature `toxicity_proxy` is REAL_COMPUTED in live V2 Redis (pre: None (MISSING)). Live value=0.40957803081044875. Source: abs(depth_imbalance) directional order-flow toxicity from live book.

## Raw evidence
`v2:features:latest:BTCUSDT:1m`.features.toxicity_proxy = 0.40957803081044875 (generated_at=2026-06-01T21:50:20Z); BTC snapshot real_feature_count=25, missing_feature_count=0; 27 live v2:features:latest:*:1m keys. Code: v2/backend/app/cli/v2_feature_pipeline_native_loop.py _features_from_market().
