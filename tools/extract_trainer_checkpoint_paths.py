...
KEYS=["checkpoint","save","load","state_dict","torch.save","torch.load","model_path","promote","best_model","version"]
...
def main()->int:
    ...
    for i,line in enumerate(txt.splitlines(),start=1):
        if rx.search(line): matches.append({"line":i,"text":line.strip()[:500],"evidence":evidence_record(str(t),i,line.strip()[:300],"checkpoint_path","trainer checkpoint keyword")})
    write_json(out/"HYBRID_TRAINER_CHECKPOINT_PATHS.json",{"matches":matches}); return 0
