...
KEYS=["confidence","probability","softmax","logits","calibration","threshold","score","entropy"]
...
def main()->int:
    ...
    for i,line in enumerate(txt.splitlines(),start=1):
        if rx.search(line): matches.append({"line":i,"text":line.strip()[:500],"evidence":evidence_record(str(t),i,line.strip()[:300],"confidence_path","trainer confidence keyword")})
    write_json(out/"HYBRID_TRAINER_CONFIDENCE_PATHS.json",{"matches":matches,"unknown_confidence_paths":0 if matches else 1}); return 0
