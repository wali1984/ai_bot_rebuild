...
KEYS=["reward","pnl","drawdown","fee","slippage","funding","liquidation","profit","loss","penalty","advantage","return"]
...
def main()->int:
    ...
    for i,line in enumerate(txt.splitlines(),start=1):
        if rx.search(line): matches.append({"line":i,"text":line.strip()[:500],"evidence":evidence_record(str(t),i,line.strip()[:300],"reward_path","trainer reward keyword")})
    write_json(out/"HYBRID_TRAINER_REWARD_PATHS.json",{"matches":matches,"unknown_reward_paths":0 if matches else 1}); return 0
