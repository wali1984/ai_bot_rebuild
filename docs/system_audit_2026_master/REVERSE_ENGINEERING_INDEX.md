...
| [PROFILED_TRAINING_OBSERVATION_COORDINATOR_STATE_V1.md](components/PROFILED_TRAINING_OBSERVATION_COORDINATOR_STATE_V1.md) | Authenticated publisher-cycle cursor, role-key commitments, exact manifest/head/epoch/page/completion addresses, crash ordering, semantic state-chain verification, and zero-authority contract. |
| [PROFILED_BASE_PUBLISHER_CYCLE_STATUS_V1.md](components/PROFILED_BASE_PUBLISHER_CYCLE_STATUS_V1.md) | Strict 57-field local publisher-status reader, canonical clocks/self-hash, ten symbol inventories and writer set algebra, file-race checks, and explicit non-authority boundary. |
| [PROFILED_TRAINING_OBSERVATION_COORDINATOR_V1.md](components/PROFILED_TRAINING_OBSERVATION_COORDINATOR_V1.md) | Crash-resuming status-to-manifest/head/witness/epoch/page/local-completion caller, exact role-key bindings, recovery order, clocks, result semantics, and non-authority boundary. |
