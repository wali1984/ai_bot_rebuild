...
|---|-----------|---------|----------|--------|
| 1 | Trainer | **Durable-learning deadlock** (first observed): validation checkpoint-promotion guard used an *absolute* 0.02 loss-increase tolerance; at the real loss scale (~8–10) it rejected promotions → `BLOCKED_NO_DURABLE_WEIGHT_UPDATE`. | **HIGH** | **PARTIALLY FIXED**: promotion tolerance made **relative** (`max(0.02, 15%×prior_loss)`) — a correct fix that stays. Streak escape added then raised to 50 (must not force-save a diverging model). |
| 2 | Trainer | **Training instability / model degradation (CAUSED BY THIS SESSION):** an entropy-floor experiment (supervised entropy bonus) **destabilized** the running model — policy entropy drifted high (~0.93) and supervised loss **diverged upward** (3.6 → 8 → 14 → 16+, val ~21). The promotion guard correctly rejects these, so the model is stuck loading a mediocre checkpoint and cannot self-recover online. | **HIGH** | **Entropy experiment REVERTED** (bonus→0.0, coef→0.01). Model is NOT self-recovering. **Requires operator recovery** (see §4.0). Older checkpoints are a smaller architecture (26MB vs current 134MB @ hidden=2048/4-block) so are **not** a valid reset target. |
...
### 4.0 Trainer recovery (operator action required)
The session's entropy experiment left the online model in a degraded, non-self-recovering state (loading a mediocre 134MB checkpoint; the promotion guard correctly refuses to persist the diverging cycles). The entropy experiment is reverted, but recovery needs one of:
1. **Offline retrain (cleanest):** retrain the current architecture (hidden=2048, 4 residual blocks) from the replay archive with a validated hyperparameter set (LR, loss-head scaling, entropy schedule). Promote the resulting checkpoint. This also fixes the original entropy-collapse without live experimentation.
2. **Temporary guard-disable for online recovery:** set `V2_TRAINER_VALIDATION_CHECKPOINT_GUARD=false` on the trainer service, let the model train down and persist recovering weights over several cycles, then re-enable the guard. (An automated persistent guard-disable was intentionally blocked as an over-reach; this is an explicit operator decision.)
3. **Lower the rejection-streak escape** (`V2_TRAINER_MAX_PROMOTION_REJECTION_STREAK=5`) so the model force-persists a low-loss cycle sooner — a middle-ground that keeps the guard but bounds the freeze.
