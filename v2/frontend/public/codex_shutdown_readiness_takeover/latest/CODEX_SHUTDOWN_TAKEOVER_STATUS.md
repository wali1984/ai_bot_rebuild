...
- `PAPER_EDGE_UNPROVEN` [P0_SHUTDOWN_BLOCKER]: post_filter_edge_pending: no unsafe post-filter fills observed, but zero fills means positive edge is not proven; remediation=`claude_v2_paper_edge_recovery_and_cost_aware_trade_selection`
- `OBSERVATORY_PAPER_EDGE_RECOVERY_REQUIRED` [P0_SHUTDOWN_BLOCKER]: observatory: decision_quality=EDGE_PENDING_INSUFFICIENT_SAMPLE paper_edge=EDGE_PENDING requires cost-aware paper edge recovery; post-filter no-fill is not positive edge; remediation=`claude_v2_paper_edge_recovery_and_cost_aware_trade_selection`
...
- descriptor: `claude_worklog/agent_supervisor/tasks/claude_v2_paper_edge_recovery_and_cost_aware_trade_selection.json`
- blocker: `PAPER_PNL_NEGATIVE_BLOCKS_CANARY`
