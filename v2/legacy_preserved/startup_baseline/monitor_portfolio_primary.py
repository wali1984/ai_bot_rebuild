#!/usr/bin/env python3
"""
PRIMARY TRADER PORTFOLIO MONITOR (Enhanced)
=============================================
...
class PrimaryPortfolioMonitor:
    def __init__(self):
        ...
        try:
            ...
            if config.BINANCE_FUT_API_KEY and config.BINANCE_FUT_API_SECRET:
                self.client = Client(api_key=config.BINANCE_FUT_API_KEY,
                                     api_secret=config.BINANCE_FUT_API_SECRET)
    ...
    def _display(self):
        ...
        print("=" * W)
        print(f"  {B}PRIMARY PORTFOLIO MONITOR{RST}  |  Account: WAJID  |  "
        ...
        # ── Position table ──
        #       Symbol       Side  Lev     Notional         Entry          Mark      Liq       Margin         PnL(ROI)  SthSL        SthTP       Trail
        hdr = (f"  {'Symbol':<13}"
               f" {'Side':<6}"
               f" {'Lev':>4}"
               f" {'Notional':>11}"
