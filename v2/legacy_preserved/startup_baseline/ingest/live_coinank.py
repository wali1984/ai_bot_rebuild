...
def _canonical_usdt(sym: str) -> str:
    ...
    if not sym.endswith("USDT"):
        sym = sym.replace("USD", "").replace("USDT", "") + "USDT"
    return sym
...
def _active_symbols_for_deep() -> list:
    ...
    """
    syms = list(COINANK_SYMBOLS)  # base list from env
    overrides = _parse_csv_env("COINANK_HOT_SYMBOLS_OVERRIDE")
    if overrides:
        syms = [_canonical_usdt(s) for s in overrides]
