def test_assemble_step_does_not_mutate_supplied_inputs():
    ...
    before = (entry.paper_trade_id, entry.symbol, entry.ledger_action, run.replay_run_id, run.symbol)

    runner.assemble_step(paper_ledger_entry=entry, replay_run=run)

    assert (entry.paper_trade_id, entry.symbol, entry.ledger_action, run.replay_run_id, run.symbol) == before
