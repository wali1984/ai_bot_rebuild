...
def test_assemble_step_propagates_service_error_for_non_paper_entry():
    ...
    with pytest.raises(ReplayBacktestRunnerServiceError) as exc_info:
        runner.assemble_step(
            paper_ledger_entry="not an entry",
            replay_run=ReplayBacktestRun("run_1", RUN_MODE_REPLAY, "BTCUSD", 1, 10, True),
        )
    assert exc_info.value.code == "must_be_paper_execution_ledger_entry"
    assert exc_info.value.field == "paper_ledger_entry"
