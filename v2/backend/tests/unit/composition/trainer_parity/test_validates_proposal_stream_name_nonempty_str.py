...
def test_validates_proposal_stream_name_nonempty_str(monkeypatch):
    calls = []
    monkeypatch.setattr(
    ...
    assert raised_type.value.field == "proposal_stream_name"
    assert raised_empty.value.code == "must_be_nonempty_str"
    assert raised_empty.value.field == "proposal_stream_name"
    assert calls == []
