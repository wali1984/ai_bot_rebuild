...
def test_evaluator_does_not_mutate_supplied_thresholds(monkeypatch):
    ...
    before = tuple(getattr(thresholds, field) for field in thresholds.__dataclass_fields__)
    before_id = id(thresholds)
    ...
    after = tuple(getattr(thresholds, field) for field in thresholds.__dataclass_fields__)
    assert id(thresholds) == before_id
    assert after == before
