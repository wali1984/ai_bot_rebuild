...
def test_evaluator_does_not_mutate_supplied_snapshot(monkeypatch):
    ...
    before = tuple(getattr(snapshot, field) for field in snapshot.__dataclass_fields__)
    before_id = id(snapshot)
    ...
    after = tuple(getattr(snapshot, field) for field in snapshot.__dataclass_fields__)
    assert id(snapshot) == before_id
    assert after == before
