...
def test_evaluator_forwards_clock_to_service(monkeypatch):
    ...
    def fake(candidate, *, thresholds, now_ms_clock):
        captured["now_ms_clock"] = now_ms_clock
        return sentinel

    monkeypatch.setattr(runtime, "evaluate_worker_health", fake)
    evaluator = build_trainer_worker_health_evaluator(thresholds=thresholds, now_ms_clock=clock)

    evaluator(snapshot)

    assert id(captured["now_ms_clock"]) == id(clock)
