...
@pytest.mark.parametrize("value", [True, 1.2, "1000"])
def test_compose_rejects_non_int_now_ms(value: object) -> None:
    with pytest.raises(TrainerLivenessCompositionError, match="now_ms"):
        compose_liveness_snapshot_with_growth(
            base_inputs(),
...
def test_compose_rejects_negative_now_ms() -> None:
    with pytest.raises(TrainerLivenessCompositionError, match="now_ms"):
        compose_liveness_snapshot_with_growth(
            base_inputs(),
            prediction_observations=(),
