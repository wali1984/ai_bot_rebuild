def test_evaluator_degraded_gpu_batch_age() -> None:
    ...
    snapshot = LivenessSignalSnapshot(123, 4096, 1000, 456, True, 1000, 899, 1000, 1000, 5, 5, False, 1000)
    result = evaluate_trainer_worker_health(snapshot, thresholds, 1000)

    assert result.status == HEALTH_STATUS_DEGRADED
    assert result.reasons == (HEALTH_REASON_GPU_BATCH_AGE_DEGRADED,)
