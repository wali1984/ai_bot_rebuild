...
def test_record_invariants_confidence_calibrated_range() -> None:
    for value, reason in ((-0.1, "must_be_in_unit_interval"), (1.1, "must_be_in_unit_interval"), (math.nan, "must_be_finite"), (math.inf, "must_be_finite"), (-math.inf, "must_be_finite")):
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(confidence_calibrated=value)
        assert (error.value.field, error.value.reason) == ("confidence_calibrated", reason)
    assert _record(confidence_calibrated=0.0).confidence_calibrated == 0.0
    assert _record(confidence_calibrated=0.5).confidence_calibrated == 0.5
    assert _record(confidence_calibrated=1.0).confidence_calibrated == 1.0
