...
def test_record_invariants_symbol_uppercase_and_charset() -> None:
    cases = [("btcusdt", "must_be_uppercase"), ("BTC USDT", "must_not_have_whitespace"), ("X" * 33, "must_be_at_most_32_chars")]
    for value, reason in cases:
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(symbol=value)
        assert (error.value.field, error.value.reason) == ("symbol", reason)
    with pytest.raises(TrainerPredictionDomainError) as error:
        _record(symbol=None)
    assert (error.value.field, error.value.reason) == ("symbol", "must_be_str")
    assert _record(symbol="BTCUSDT").symbol == "BTCUSDT"
    assert len(_record(symbol="X" * 32).symbol) == 32
