...
def test_record_invariants_top_negative_feature_codes() -> None:
    for value in (["a"], {"a"}, {"a": 1}):
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(top_negative_feature_codes=value)
        assert (error.value.field, error.value.reason) == ("top_negative_feature_codes", "must_be_tuple")
    ...
    for value, reason in cases:
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(top_negative_feature_codes=value)
        assert (error.value.field, error.value.reason) == ("top_negative_feature_codes", reason)
    assert _record(top_negative_feature_codes=()).top_negative_feature_codes == ()
