...
def test_record_invariants_confidence_calibrated_type() -> None:
    for value in (0, False):
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(confidence_calibrated=value)
        assert (error.value.field, error.value.reason) == ("confidence_calibrated", "must_be_float")
    assert _record(confidence_calibrated=0.25).confidence_calibrated == 0.25
