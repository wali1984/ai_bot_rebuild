...
def test_record_invariants_direction_in_allowed() -> None:
    assert _record(direction="long").direction == "long"
    assert _record(direction="short").direction == "short"
    assert _record(direction="flat").direction == "flat"
    with pytest.raises(TrainerPredictionDomainError) as error:
        _record(direction="up")
    assert (error.value.field, error.value.reason) == ("direction", "invalid_direction")
    with pytest.raises(TrainerPredictionDomainError) as error:
        _record(direction=1)
    assert (error.value.field, error.value.reason) == ("direction", "must_be_str")
