...
def test_record_invariants_confidence_raw_type() -> None:
    for value in (0, False):
        with pytest.raises(TrainerPredictionDomainError) as error:
            _record(confidence_raw=value)
        assert (error.value.field, error.value.reason) == ("confidence_raw", "must_be_float")
    assert _record(confidence_raw=0.25).confidence_raw == 0.25
