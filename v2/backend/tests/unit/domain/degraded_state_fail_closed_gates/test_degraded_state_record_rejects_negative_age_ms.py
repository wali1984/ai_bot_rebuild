...
def test_smc_age_ms_negative_rejected() -> None:
    with pytest.raises(DegradedStateFailClosedGatesDomainError):
        DegradedStateRecord(**_kwargs(smc_age_ms=-1))  # type: ignore[arg-type]


def test_liq_age_ms_negative_rejected() -> None:
    with pytest.raises(DegradedStateFailClosedGatesDomainError):
        DegradedStateRecord(**_kwargs(liq_age_ms=-1))  # type: ignore[arg-type]


def test_oi_age_ms_negative_rejected() -> None:
    with pytest.raises(DegradedStateFailClosedGatesDomainError):
        DegradedStateRecord(**_kwargs(oi_age_ms=-1))  # type: ignore[arg-type]


def test_orderbook_age_ms_negative_rejected() -> None:
    with pytest.raises(DegradedStateFailClosedGatesDomainError):
        DegradedStateRecord(**_kwargs(orderbook_age_ms=-1))  # type: ignore[arg-type]
