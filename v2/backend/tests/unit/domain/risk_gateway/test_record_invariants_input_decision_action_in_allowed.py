import pytest

from v2.backend.app.domain.risk_gateway import (
    RISK_DECISION_ACTION_DENY,
    RISK_DECISION_REASON_DENY_DEFAULT,
    RiskDecisionRecord,
    RiskGatewayDomainError,
)


def test_input_decision_action_must_be_string_member_of_allowed_set() -> None:
    def build(value: object) -> None:
        RiskDecisionRecord(
            risk_decision_id="risk-1",
            decision_id="decision-1",
            prediction_id="prediction-1",
            feature_snapshot_id="feature-1",
            symbol="BTCUSDT",
            risk_decision_ts_ms=1,
            risk_action=RISK_DECISION_ACTION_DENY,
            risk_reason_code=RISK_DECISION_REASON_DENY_DEFAULT,
            input_decision_action=value,
            input_decision_reason_code="proceed_long",
            live_blocked=True,
        )

    with pytest.raises(RiskGatewayDomainError) as exc_info:
        build("sideways")
    assert exc_info.value.field == "input_decision_action"
    assert exc_info.value.reason == "invalid_input_decision_action"
    with pytest.raises(RiskGatewayDomainError) as exc_info:
        build(42)
    assert exc_info.value.field == "input_decision_action"
    assert exc_info.value.reason == "must_be_str"
