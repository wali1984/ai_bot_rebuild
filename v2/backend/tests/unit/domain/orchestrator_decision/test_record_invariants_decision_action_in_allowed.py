...
def test_record_rejects_decision_action_outside_allowed_set():
    base = {
    ...
    for value in ("OPEN_LONG", "buy", ""):
        with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
            OrchestratorDecisionRecord(**{**base, "decision_action": value})
        assert exc_info.value.field == "decision_action"
        assert exc_info.value.reason == "invalid_decision_action"
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(**{**base, "decision_action": 42})
