...
def test_record_rejects_decision_reason_code_outside_allowed_set():
    base = {
    ...
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(
            **{**base, "decision_reason_code": "proceed_neutral"}
        )
    assert exc_info.value.field == "decision_reason_code"
    assert exc_info.value.reason == "invalid_decision_reason_code"
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(**{**base, "decision_reason_code": 42})
