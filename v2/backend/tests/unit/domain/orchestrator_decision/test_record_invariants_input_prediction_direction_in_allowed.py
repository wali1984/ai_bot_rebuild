...
def test_record_rejects_input_prediction_direction_outside_allowed_set():
    base = {
    ...
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(
            **{**base, "input_prediction_direction": "sideways"}
        )
    assert exc_info.value.field == "input_prediction_direction"
    assert exc_info.value.reason == "invalid_input_prediction_direction"
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(**{**base, "input_prediction_direction": 42})
