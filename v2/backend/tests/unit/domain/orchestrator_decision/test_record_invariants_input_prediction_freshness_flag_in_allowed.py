...
def test_record_rejects_input_prediction_freshness_flag_outside_allowed_set():
    base = {
    ...
    for value in ("unknown", "FRESH", ""):
        with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
            OrchestratorDecisionRecord(
                **{**base, "input_prediction_freshness_flag": value}
            )
        assert exc_info.value.reason == "invalid_input_prediction_freshness_flag"
    with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
        OrchestratorDecisionRecord(
            **{**base, "input_prediction_freshness_flag": 42}
        )
