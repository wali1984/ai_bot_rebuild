...
def test_record_validates_input_prediction_confidence_calibrated_range():
    base = {
    ...
    for value in (-0.0001, 1.0001):
        ...
        assert exc_info.value.reason == "must_be_in_unit_interval"
    for value in (float("nan"), float("inf"), float("-inf")):
        with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
            OrchestratorDecisionRecord(
                **{**base, "input_prediction_confidence_calibrated": value}
            )
        assert exc_info.value.reason == "must_be_finite"
