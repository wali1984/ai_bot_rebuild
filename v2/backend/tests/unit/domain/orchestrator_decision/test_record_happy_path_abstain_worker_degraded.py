from v2.backend.app.domain.orchestrator_decision import (
    DECISION_ACTION_ABSTAIN,
    DECISION_REASON_ABSTAIN_WORKER_DEGRADED,
    OrchestratorDecisionRecord,
)


def test_record_happy_path_abstain_worker_degraded_accepts_valid_inputs():
    record = OrchestratorDecisionRecord(
        decision_id="decision-abstain-degraded",
        prediction_id="prediction-abstain-degraded",
        feature_snapshot_id="snapshot-abstain-degraded",
        symbol="ADAUSDT",
        decision_ts_ms=106,
        decision_action=DECISION_ACTION_ABSTAIN,
        decision_reason_code=DECISION_REASON_ABSTAIN_WORKER_DEGRADED,
        input_prediction_direction="long",
        input_prediction_confidence_calibrated=0.60,
        input_prediction_freshness_flag="fresh",
        input_worker_health_status="DEGRADED",
        live_blocked=True,
    )
    assert record.input_worker_health_status == "DEGRADED"
