...
def test_record_rejects_invalid_feature_snapshot_id_charset_and_length():
    base = {
    ...
    for value, reason in cases:
        with pytest.raises(OrchestratorDecisionDomainError) as exc_info:
            OrchestratorDecisionRecord(**{**base, "feature_snapshot_id": value})
        assert exc_info.value.field == "feature_snapshot_id"
