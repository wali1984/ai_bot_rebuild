import pytest

from v2.backend.app.domain.replay_backtest_runner import ReplayBacktestRun, ReplayBacktestRunnerDomainError


def test_run_rejects_run_ended_ts_ms_before_run_started_ts_ms():
    with pytest.raises(ReplayBacktestRunnerDomainError) as error:
        ReplayBacktestRun(
        replay_run_id="run-1",
        run_mode="replay",
        symbol="BTCUSDT",
        run_started_ts_ms=100,
        run_ended_ts_ms=99,
        live_blocked=True,
        )
    assert error.value.field == "run_ended_ts_ms"
    assert error.value.reason == "run_ended_ts_ms_must_be_ge_run_started_ts_ms"
