...
def test_evaluate_appends_proposal_observation_to_proposal_history():
    prior = (StreamIdObservation("prop", "1-0", 900),)
    result = evaluate_trainer_liveness(
        _FakeReader({"pred": None, "prop": "2-0"}),
    ...
    assert result.proposal_history[-1].stream_name == "prop"
    assert result.proposal_history[-1].stream_id == "2-0"
    assert len(result.proposal_history) == len(prior) + 1
