...
def test_portfolio_level_buffer_computed():
    snap = build_portfolio_liquidation_snapshot(
        account=_account(), positions=_positions(), generated_utc=NOW
    )
    ...
    assert all(
        row["estimated_position_liquidation_price"] is not None
        for row in snap["position_liquidation_register"]
    )
...
def test_canonical_paper_position_aliases_are_not_dropped_or_forced_to_one_x():
    positions = [
        {
            "symbol": "MANAUSDT",
            "net_quantity": 1000.0,
    ...
    snap = build_portfolio_liquidation_snapshot(
        account=_account(totalMaintMargin=0.36),
        positions=positions,
        generated_utc=NOW,
    )

    assert snap["open_position_count"] == 1
    assert snap["maintenance_margin_evidence_complete"] is True
    assert snap["leverage_evidence_complete"] is True
    register = snap["position_liquidation_register"][0]
    assert register["symbol"] == "MANAUSDT"
    assert register["leverage"] == 2.0
    assert register["isolated"] is True
    assert snap["positions"][0]["position_amt"] == 1000.0
    assert snap["positions"][0]["mark_price"] == 0.072


def test_missing_leverage_is_reported_not_silently_stamped_as_one_x():
    position = {
        "symbol": "ARBUSDT",
        "net_quantity": 100.0,
        "mark_price": 0.1,
    ...
    snap = build_portfolio_liquidation_snapshot(
        account=_account(totalMaintMargin=0.05),
        positions=[position],
        generated_utc=NOW,
    )

    assert snap["open_position_count"] == 1
    assert snap["leverage_evidence_complete"] is False
    assert snap["leverage_evidence_missing_symbols"] == ["ARBUSDT"]
    assert snap["position_liquidation_register"][0]["leverage"] is None
