...
def test_missing_decision_time_falls_back_to_input_order() -> None:
    exs = [_ex("BTCUSDT", "1m", None, t) for t in range(1, 4)]
    out = build_example_windows(exs, seq_len=2)
    # Input order preserved as the monotonic proxy: windows are [1,1],[1,2],[2,3].
    assert [f[0] for f in out[2].window] == [2.0, 3.0]
