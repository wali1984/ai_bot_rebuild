...
def test_assemble_abstain_worker_critical() -> None:
    record = assemble_orchestrator_decision_record(
        prediction=TrainerPredictionRecord(
            prediction_id="pred_worker_critical",
            feature_snapshot_id="snap_worker_critical",
            symbol="BTCUSDT",
            model_version="model",
    ...
    assert record.decision_reason_code == "abstain_worker_critical"
