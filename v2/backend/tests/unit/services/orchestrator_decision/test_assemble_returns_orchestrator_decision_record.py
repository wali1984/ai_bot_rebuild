...
def test_assemble_returns_orchestrator_decision_record() -> None:
    record = assemble_orchestrator_decision_record(
        prediction=TrainerPredictionRecord(
            prediction_id="pred_record_type",
            feature_snapshot_id="snap_record_type",
            symbol="BTCUSDT",
            model_version="model",
    ...
    assert isinstance(record, OrchestratorDecisionRecord)
