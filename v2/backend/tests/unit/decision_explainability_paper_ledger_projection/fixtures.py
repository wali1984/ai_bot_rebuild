...
PAPER_LEDGER_CLOCK_START_MS = BASE_TS_MS + 7_000_000

SCENARIO_BTC_SLUG = "paper_ledger_explainability_pack_btc_winner_long"
SCENARIO_ETH_SLUG = "paper_ledger_explainability_pack_eth_winner_short"
SCENARIO_LAB_SLUG = "paper_ledger_explainability_pack_lab_loser_short"
SCENARIO_SOL_SLUG = "paper_ledger_explainability_pack_sol_orchestrator_held"
...
def build_paper_ledger_explainability_pack_btc_winner_long() -> tuple[
    PaperLedgerExplainabilityFixtureInput, ...
]:
    return _build_scenario_inputs(scenario_index=0)


def build_paper_ledger_explainability_pack_eth_winner_short() -> tuple[
    PaperLedgerExplainabilityFixtureInput, ...
]:
    return _build_scenario_inputs(scenario_index=1)


def build_paper_ledger_explainability_pack_lab_loser_short() -> tuple[
    PaperLedgerExplainabilityFixtureInput, ...
]:
    return _build_scenario_inputs(scenario_index=2)


def build_paper_ledger_explainability_pack_sol_orchestrator_held() -> tuple[
    PaperLedgerExplainabilityFixtureInput, ...
]:
    return _build_scenario_inputs(scenario_index=3)


def build_paper_ledger_explainability_fixture_inputs() -> tuple[
    PaperLedgerExplainabilityFixtureInput, ...
]:
    rows: list[PaperLedgerExplainabilityFixtureInput] = []
...
def _build_scenario_inputs(
    *, scenario_index: int
) -> tuple[PaperLedgerExplainabilityFixtureInput, ...]:
    (
        scenario_slug,
        symbol,
        risk_action,
        ...
        rows.append(
            PaperLedgerExplainabilityFixtureInput(
                ...
                legacy_evidence_pointer=(
                    f"legacy_evidence__paper_ledger_explainability__"
                    f"{pointer_suffix}__step_{step_ordinal}"
                ),
                has_lab_pointer=has_lab_pointer,
                risk_decision_record=RiskDecisionRecord(
                    risk_decision_id=(
                        f"risk_decision_phase2s_{scenario_slug}_{ordinal_id}"
                    ),
                    decision_id=f"decision_phase2s_{scenario_slug}_{ordinal_id}",
                    prediction_id=f"prediction_phase2s_{scenario_slug}_{ordinal_id}",
                    feature_snapshot_id=(
                        f"feature_snapshot_phase2s_{scenario_slug}_{ordinal_id}"
                    ),
                    symbol=symbol,
                    risk_decision_ts_ms=(
