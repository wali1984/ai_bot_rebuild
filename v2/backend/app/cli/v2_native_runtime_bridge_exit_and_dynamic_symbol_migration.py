"""V2 native-runtime bridge-exit and dynamic-symbol migration CLI.

Analysis-only. Emits the nine-phase packet under
...
from v2.backend.app.services.bridge_exit.native_runtime_bridge_exit import (  # noqa: E402
    default_paths,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the V2 native-runtime bridge-exit and dynamic-symbol "
            "migration planner (analysis-only)."
        ),
    )
    parser.add_argument(
        "--repo-root",
