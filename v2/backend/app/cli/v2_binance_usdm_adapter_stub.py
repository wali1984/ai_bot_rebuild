"""V2 Binance USD-M futures adapter stub — CLI worker.

This is the worker-layer refusal surface for the Binance USD-M futures
mutation operations and the presence-only observation surface for the
two Binance USD-M read-only operations (``account_info_v3``,
``position_risk``). It sits above the API-layer
``LiveBlockGuardMiddleware``
(``v2/backend/app/api/middleware/live_block_guard.py``).

The worker:
  * instantiates :class:`BinanceUsdmAdapter`, whose five mutation
    methods all raise :class:`BlockedGateNotApprovedError` with code
    ``BLOCKED_GATE_NOT_APPROVED`` before evaluating any argument;
  * exposes the two read-only methods (``account_info_v3``,
    ``position_risk``) as presence-only observations that never make a
    real exchange call and never return or log the credential value;
  ...
  * emits a single public status payload exposing only the adapter's
    own disabled state and the presence-of-credentials boolean — never
    any exchange data and never the credential value.
...
Hard rules (asserted by tests):
  ...
  - The worker and service source contain no literal Binance REST
    mutation method references.
  - The stub state is one of ``DISABLED`` or ``BLOCKED``, never
    ``ACTIVE``.
  - The live gate is permanently ``blocked_human_only``; no codepath
    here can change it. Read-only access does NOT unlock the gate.
  - No secret value is ever returned by any method, logged by the
    worker, or written to any output payload.
...
WORKER_ID = "v2_binance_usdm_adapter"
...
EXCHANGE_CALL_INVARIANT = "NO_REAL_EXCHANGE_CALL_FROM_BINANCE_USDM_ADAPTER_STUB"
SECRET_LEAK_INVARIANT = "NO_CREDENTIAL_VALUE_RETURNED_OR_LOGGED_BY_ANY_METHOD"
...
# refusal surface back to the legacy Binance USD-M operations it
# replaces.
LEGACY_BINANCE_USDM_SOURCE_PATHS: List[str] = [
...
REQUIRED_PUBLIC_PAYLOAD_FIELDS: Tuple[str, ...] = (
    ...
    "endpoints_exposed_read_only",
    "endpoints_blocked_mutating",
    "blocked_call_attempts_total",
    "blocked_call_breakdown_by_method",
    "readonly_call_attempts_total",
    "readonly_call_breakdown_by_method",
    "credentials_present_in_env",
    "credential_env_keys_checked",
    "credentials_returned_by_any_method",
    "credentials_logged_by_any_method",
    "secret_leak_invariant",
    "exchange_call_invariant",
    "exchange_call_taken",
    "exchange_client_present",
    "error_code_on_call",
    "live_gate",
    "current_gate_state",
    "current_gate_state_must_equal_blocked_human_only",
    "gate_always_blocked_invariant",
    "live_gate_unlocked_by_readonly_access",
    "live_blocked",
    "fail_closed",
    "freshness_seconds",
    "legacy_binance_usdm_source_paths",
    "legacy_readonly_rest_paths_documented_only",
...
def build_status(
    *,
    ...
    status: Dict[str, Any] = {
        ...
        "readonly_methods": snapshot["readonly_methods"],
        "endpoints_exposed_read_only": snapshot["readonly_methods"],
        "endpoints_blocked_mutating": snapshot["mutation_methods"],
        "blocked_call_attempts_total": snapshot["blocked_call_attempts_total"],
        "blocked_call_breakdown_by_method": snapshot[
            "blocked_call_breakdown_by_method"
        ],
        "readonly_call_attempts_total": snapshot["readonly_call_attempts_total"],
        "readonly_call_breakdown_by_method": snapshot[
            "readonly_call_breakdown_by_method"
        ],
        "credentials_present_in_env": snapshot["credentials_present_in_env"],
        "credential_env_keys_checked": snapshot["credential_env_keys_checked"],
        "credentials_returned_by_any_method": False,
        "credentials_logged_by_any_method": False,
        "secret_leak_invariant": SECRET_LEAK_INVARIANT,
        ...
        "fail_closed": True,
        "freshness_seconds": 0,
        "legacy_binance_usdm_source_paths": list(LEGACY_BINANCE_USDM_SOURCE_PATHS),
