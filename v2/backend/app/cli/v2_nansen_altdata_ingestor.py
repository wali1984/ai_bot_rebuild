"""V2 Nansen alternative-data ingestor CLI (paper/shadow only).

Bounded one-shot ingestor that calls the Nansen free-tier paper/shadow
client for a fixed symbol set and publishes results to v2:altdata:*.
Without NANSEN_API_KEY in the process env or local custody vault, this
CLI emits a KEY_MISSING_NO_NETWORK status and exits without opening any
network connection.
...
from v2.backend.app.services.alternative_data.nansen_client import (
    DEFAULT_VAULT_PATH,
    KEY_STATUS,
    NansenClient,
    RateLimitState,
...
GO_READY = "V2_NANSEN_FREE_TIER_CLIENT_PAPER_SHADOW_READY"
GO_BLOCKED = "V2_NANSEN_FREE_TIER_CLIENT_PAPER_SHADOW_BLOCKED"
...
def run_once(
    *,
    ...
    """Execute one bounded ingestion pass.

    The caller passes in (or omits) http_get / now_ms_func / budget /
    ttl / cooldown so tests can drive the client deterministically.
    """
    ...
    client = NansenClient(**client_kwargs)
    results = []
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="v2_nansen_altdata_ingestor")
    parser.add_argument(
        "--symbols",
        default=None,
        help=(
