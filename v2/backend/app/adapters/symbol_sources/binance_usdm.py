...
class BinanceUsdMFuturesSource:
    source = "binance_usdm"

    def __init__(self, base_endpoint: str = "https://fapi.binance.com", redis_client: Any | None = None) -> None:
        self.base_endpoint = base_endpoint.rstrip("/")
        self.redis_client = redis_client

    @property
    def exchange_info_url(self) -> str:
        return f"{self.base_endpoint}/fapi/v1/exchangeInfo"
    ...
    def _fetch_exchange_info_from_cache(self) -> Dict[str, Any] | None:
        ...
        for key in (
            ...
            if _payload_has_symbols(payload):
                return {
                    **dict(payload),
                    "source": str(payload.get("source") or "binance_usdm_websocket_cache_primary"),
    ...
    def fetch_exchange_info(self, timeout: float = 10.0) -> Dict[str, Any]:
        ...
        try:
            require_binance_rest_fallback(
                ...
                fallback_reason="binance_usdm_symbol_source_cache_missing",
        ...
        if isinstance(payload, dict):
            payload["source"] = "binance_usdm_exchangeInfo_rest_fallback"
