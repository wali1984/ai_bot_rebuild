...
_HIGH_WATER_KEYS = frozenset(
    {
        ...
        "candidate_archive_row_count",
        "candidate_archive_terminal_chain_sha256",
        "candidate_archive_writer_id",
        "candidate_archive_writer_public_key_hex",
    }
)
_SPLIT_BOUNDARY_KEYS = frozenset(
    {
        "embargo_groups_before_each_downstream_split",
        "holdout_start_decision_time",
        "holdout_start_row_fraction",
        "minimum_holdout_rows",
        "minimum_validation_rows",
        "validation_start_decision_time",
        "validation_start_row_fraction",
    }
)
_BOUNDARY_FRACTION_PAIRS = frozenset(
    {
        (0.50, 0.80),
        (0.45, 0.75),
        (0.40, 0.70),
...
_PURGE_REASONS = frozenset(
    {
        "FEATURE_GROUP_CROSSES_SPLIT_BOUNDARY",
        "PREDECLARED_DECISION_TIME_EMBARGO",
        "TRAIN_LABEL_NOT_AVAILABLE_BEFORE_VALIDATION",
        "VALIDATION_LABEL_NOT_AVAILABLE_BEFORE_HOLDOUT",
    }
)
_CANDIDATE_SOURCE_HASH_KEYS = frozenset(
    {
        "candidate_archive_terminal_chain_sha256",
...
_GEN5_SOURCE_HASH_KEYS = frozenset(
    {
        "canonical_label_binding_sha256",
        "cost_capture_artifact_sha256",
        "cost_capture_binding_sha256",
        "cost_capture_receipt_sha256",
        "cost_cas_object_inventory_sha256",
        "mtf_binding_sha256",
        "parent_lineage_binding_sha256",
        "parent_record_sha256",
        "profiled_ledger_high_water_sha256",
        "profiled_ledger_record_sha256",
    }
)
...
def _load_pinned_base_dataset(
    receipt: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    path_value = receipt.get("base_dataset_path")
    if type(path_value) is not str:
        _fail("ABSOLUTE_PATH_WITHOUT_TRAVERSAL_REQUIRED", "build_receipt.base_dataset_path")
    path = Path(path_value)
    base, file_sha = _read_regular_object(path, "build_receipt.base_dataset_path")
    if (
        file_sha != PINNED_BASE_DATASET_FILE_SHA256
        or receipt.get("base_dataset_file_sha256") != file_sha
        or receipt.get("trusted_base_dataset_file_sha256") != file_sha
        or set(base) != _DATASET_KEYS
        or base.get("schema_version") != "serving_compatible_dataset_v2"
    ):
        _fail("PINNED_BASE_DATASET_REQUIRED", "build_receipt.base_dataset_path")
    base_sha = _sha256(base.get("dataset_sha256"), "base_dataset.dataset_sha256")
    material = {
        key: value for key, value in base.items() if key not in {"dataset_id", "dataset_sha256"}
    }
    if (
        canonical_sha256(material) != base_sha
        or base.get("dataset_id") != f"serving_dataset_v2_{base_sha[:24]}"
        or base.get("feature_abi_sha256") != feature_abi_sha256()
        or base.get("feature_builder_sha256") != feature_builder_sha256()
        or tuple(base.get("ordered_feature_names") or ()) != ORDERED_FEATURE_NAMES
        or tuple(base.get("action_labels") or ()) != ACTION_LABELS
    ):
        _fail("BASE_DATASET_CONTENT_OR_CONTRACT_MISMATCH", "base_dataset")
    raw_rows = base.get("rows")
    if type(raw_rows) is not list or not raw_rows:
        _fail("NONEMPTY_ROWS_REQUIRED", "base_dataset.rows")
    expected_rows: dict[str, dict[str, Any]] = {}
    for index, raw_row in enumerate(raw_rows):
        if type(raw_row) is not dict:
            _fail("OBJECT_REQUIRED", f"base_dataset.rows[{index}]")
        row = dict(raw_row)
        row_id = _text(row.get("row_id"), f"base_dataset.rows[{index}].row_id")
        if row_id in expected_rows:
            _fail("DUPLICATE_ROW_ID", "base_dataset.rows")
        row.pop("split", None)
        row["feature_group_id"] = row.get("snapshot_id")
        ...
        expected_rows[row_id] = row
    return base, expected_rows
...
def _validate_row(
    ...
    index: int,
    *,
    archive_terminal_chain_sha256: str,
) -> dict[str, Any]:
    ...
    expected_source_hash_keys = (
        _CANDIDATE_SOURCE_HASH_KEYS
        if source_kind == "CANDIDATE_DECISION_OUTCOME_V2"
        else _GEN5_SOURCE_HASH_KEYS
    )
    if set(source_hashes) != expected_source_hash_keys:
        _fail("EXACT_SOURCE_HASH_SCHEMA_REQUIRED", f"{field}.source_hashes")
    ...
    if source_kind == "CANDIDATE_DECISION_OUTCOME_V2":
        ...
        if (
            set(derivation) != _DERIVATION_KEYS
            ...
            or derivation.get("proposed_action") not in {"LONG", "SHORT", "HOLD"}
            or derivation.get("derivation_method")
            not in {
        ...
        if canonical_sha256(derivation_material) != derivation.get("derivation_sha256"):
            _fail("DIRECTIONAL_LABEL_DERIVATION_SHA256_MISMATCH", field)
        ...
        label_material = {
            ...
            "label_record_available_at_ms": int(label.timestamp() * 1_000),
            "directional_label_derivation_sha256": derivation["derivation_sha256"],
        ...
        if (
            ...
            != row.get("label_binding_sha256")
            or source_hashes.get("feature_snapshot_content_sha256")
            != row.get("source_content_sha256")
            or source_hashes.get("candidate_archive_terminal_chain_sha256")
            != archive_terminal_chain_sha256
        ):
            _fail("CANDIDATE_SOURCE_LINEAGE_MISMATCH", field)
        _text(row.get("decision_disposition"), f"{field}.decision_disposition")
        _text(row.get("eventual_disposition"), f"{field}.eventual_disposition")
    else:
        if (
        ...
        if (
            source_hashes.get("canonical_label_binding_sha256")
            != row.get("label_binding_sha256")
        ):
            _fail("GEN5_LABEL_SOURCE_LINEAGE_MISMATCH", field)
    return row
...
def load_validated_training_artifacts(
    *,
    ...
    feature_archive_root = receipt.get("feature_archive_root")
    if (
        type(feature_archive_root) is not str
        or not Path(feature_archive_root).is_absolute()
        or ".." in Path(feature_archive_root).parts
    ):
        _fail("SAFE_FEATURE_ARCHIVE_ROOT_REQUIRED", "build_receipt.feature_archive_root")
    ...
    if (
        set(archive) != _ARCHIVE_KEYS
        or archive.get("schema_version") != ARCHIVE_VERIFICATION_SCHEMA_VERSION
    ...
    base_dataset, expected_base_rows = _load_pinned_base_dataset(receipt)
    archive_bindings = {
    ...
    rows = [
        _validate_row(
            row,
            index,
            archive_terminal_chain_sha256=archive_terminal_sha,
        )
    ...
    admitted_base_row_ids: set[str] = set()
    for row in rows:
        source_split_counts[row["source_kind"]][row["split"]] += 1
        feature_group_splits[row["feature_group_id"]].add(row["split"])
        feature_group_counts[row["feature_group_id"]] += 1
        admitted_receipts.update(row["source_receipt_sha256s"])
        if row["source_kind"] == "GEN5_AUTHENTICATED_PROFILED_OBSERVATION":
            expected = expected_base_rows.get(row["row_id"])
            if expected is None:
                _fail("GEN5_ROW_NOT_IN_PINNED_BASE_DATASET", "dataset.rows")
            expected = {**expected, "split": row["split"]}
            if row != expected:
                _fail("GEN5_ROW_DIFFERS_FROM_PINNED_BASE_DATASET", "dataset.rows")
            admitted_base_row_ids.add(row["row_id"])
    if len(admitted_base_row_ids) != source_counts["GEN5_AUTHENTICATED_PROFILED_OBSERVATION"]:
        _fail("DUPLICATE_GEN5_BASE_ROW", "dataset.rows")
    ...
    manifest_action_counts = _counter(
        manifest.get("target_action_counts"), "manifest.target_action_counts"
    )
    manifest_symbol_counts = _counter(
        manifest.get("symbol_counts"), "manifest.symbol_counts"
    )
    manifest_timeframe_counts = _counter(
        manifest.get("timeframe_counts"), "manifest.timeframe_counts"
    )
    manifest_source_counts = _counter(
        manifest.get("source_row_counts"), "manifest.source_row_counts"
    )
    raw_source_split_counts = manifest.get("source_split_counts")
    if (
        not isinstance(raw_source_split_counts, Mapping)
        or set(raw_source_split_counts) != _SOURCE_KINDS
    ):
        _fail("SOURCE_SPLIT_COUNT_SCHEMA_MISMATCH", "manifest.source_split_counts")
    manifest_source_split_counts = {
        source: _counter(
            raw_source_split_counts[source],
            f"manifest.source_split_counts.{source}",
        )
        for source in sorted(_SOURCE_KINDS)
    }
    if any(set(counts) != set(_SPLITS) for counts in manifest_source_split_counts.values()):
        _fail("SOURCE_SPLIT_COUNT_SCHEMA_MISMATCH", "manifest.source_split_counts")
    if (
        manifest_action_counts
        != {action: action_counts[action] for action in ACTION_LABELS}
        or manifest_symbol_counts != dict(sorted(symbol_counts.items()))
        or manifest_timeframe_counts != dict(sorted(timeframe_counts.items()))
        or manifest_source_counts != dict(sorted(source_counts.items()))
        or manifest_source_split_counts != source_split_counts
        or _nonnegative_int(manifest.get("symbol_count"), "manifest.symbol_count")
        != len(symbol_counts)
        or _nonnegative_int(manifest.get("timeframe_count"), "manifest.timeframe_count")
        != len(timeframe_counts)
    ):
        _fail("MANIFEST_ROW_COUNTER_MISMATCH", "manifest")
    if (
        _nonnegative_int(
            manifest.get("feature_group_count"), "manifest.feature_group_count"
        )
        != len(feature_group_counts)
        or _nonnegative_int(
            manifest.get("reused_feature_group_count"),
            "manifest.reused_feature_group_count",
        )
        != sum(count > 1 for count in feature_group_counts.values())
        or _nonnegative_int(
            manifest.get("maximum_rows_per_feature_group"),
            "manifest.maximum_rows_per_feature_group",
        )
        != max(feature_group_counts.values())
    ):
        _fail("FEATURE_GROUP_COUNTER_MISMATCH", "manifest")
    if (
        ...
        or _nonnegative_int(
            manifest.get("source_receipt_sha256_count"),
            "manifest.source_receipt_sha256_count",
        )
        != len(admitted_receipts)
    ):
    ...
    if not set(purge_counts) <= _PURGE_REASONS:
        _fail("UNKNOWN_PURGE_REASON", "manifest.purge_reason_counts")
    rejection_counts = _counter(
        manifest.get("candidate_exclusion_reasons"),
        "manifest.candidate_exclusion_reasons",
    )
    def rejected_for(*markers: str) -> int:
        return sum(
            count
    ...
    derived_rejection_counters = {
        "future_time_rejections": rejected_for(
            "AFTER_DECISION",
            "FUTURE_TIME",
            "POINT_IN_TIME_ORDER_INVALID",
            "POINT_IN_TIME_CLOCK_ORDER_INVALID",
        ),
        "finality_unproven": rejected_for("FINALITY", "UNCLOSED", "CLOSED_KLINE"),
        "missing_cost_evidence": rejected_for(
            "MISSING_COST", "COST_EVIDENCE_REQUIRED"
        ),
        "missing_label_evidence": rejected_for(
    ...
    if any(
        _nonnegative_int(manifest.get(field), f"manifest.{field}") != expected
        for field, expected in derived_rejection_counters.items()
    ):
        _fail("DERIVED_REJECTION_COUNTER_MISMATCH", "manifest")
    ...
    if (
        ...
        or _nonnegative_int(
            manifest.get("candidate_rejection_count"),
            "manifest.candidate_rejection_count",
        )
    ...
    if (
        ...
        or any(type(row_id) is not str or not row_id for row_id in purged_ids)
        or any(type(row_id) is not str or not row_id for row_id in embargo_ids)
        or purged_ids != sorted(purged_ids)
        or embargo_ids != sorted(embargo_ids)
        ...
        or _nonnegative_int(
            manifest.get("embargo_row_count"), "manifest.embargo_row_count"
        )
        != len(embargo_ids)
        or _nonnegative_int(
            manifest.get("embargo_groups"), "manifest.embargo_groups"
        )
        != EMBARGO_GROUPS
    ):
        _fail("PURGE_EMBARGO_ACCOUNTING_MISMATCH", "manifest")
    boundaries = manifest.get("split_boundaries")
    combined_rows_before_purge = len(expected_base_rows) + candidate_eligible
    if not isinstance(boundaries, Mapping) or set(boundaries) != _SPLIT_BOUNDARY_KEYS:
        _fail("SPLIT_BOUNDARY_SCHEMA_MISMATCH", "manifest.split_boundaries")
    validation_fraction = _finite(
        boundaries.get("validation_start_row_fraction"),
        "manifest.split_boundaries.validation_start_row_fraction",
    )
    holdout_fraction = _finite(
        boundaries.get("holdout_start_row_fraction"),
        "manifest.split_boundaries.holdout_start_row_fraction",
    )
    validation_boundary = _utc(
        boundaries.get("validation_start_decision_time"),
        "manifest.split_boundaries.validation_start_decision_time",
    )
    holdout_boundary = _utc(
        boundaries.get("holdout_start_decision_time"),
        "manifest.split_boundaries.holdout_start_decision_time",
    )
    if (
        type(boundaries.get("validation_start_row_fraction")) is not float
        or type(boundaries.get("holdout_start_row_fraction")) is not float
        or (validation_fraction, holdout_fraction) not in _BOUNDARY_FRACTION_PAIRS
        or validation_boundary >= holdout_boundary
        or _nonnegative_int(
            boundaries.get("minimum_validation_rows"),
            "manifest.split_boundaries.minimum_validation_rows",
        )
        != max(10, int(combined_rows_before_purge * 0.05))
        or _nonnegative_int(
            boundaries.get("minimum_holdout_rows"),
            "manifest.split_boundaries.minimum_holdout_rows",
        )
        != max(10, int(combined_rows_before_purge * 0.05))
        or _nonnegative_int(
            boundaries.get("embargo_groups_before_each_downstream_split"),
            "manifest.split_boundaries.embargo_groups_before_each_downstream_split",
        )
        != EMBARGO_GROUPS
        or manifest.get("purge_policy") != PURGE_POLICY
    ):
        _fail("SPLIT_BOUNDARY_OR_PURGE_POLICY_MISMATCH", "manifest")
    ...
    if (
        max(_utc(row["decision_time"], "decision_time") for row in train_rows)
        >= validation_boundary
        or min(
        ...
        >= holdout_boundary
        or min(_utc(row["decision_time"], "decision_time") for row in holdout_rows)
        < holdout_boundary
    ):
        _fail("SPLIT_BOUNDARY_ROW_ASSIGNMENT_MISMATCH", "dataset")
    ...
    if (
        ...
        or type(parity.get("required_feature_missing_rate")) is not float
        or parity.get("required_feature_missing_rate") != 0.0
        or parity.get("activation_eligible") is not False
        or parity.get("activation_block_reason")
        != "CURRENT_SERVING_DISTRIBUTION_NOT_YET_EVALUATED"
        or parity.get("paper_only") is not True
        or parity.get("live_eligible") is not False
        or any(
            _nonnegative_int(parity.get(field), f"parity.{field}")
            != split_counts[split]
            for split, field in split_fields.items()
        )
    ):
    ...
    for field in split_fields.values():
        if _nonnegative_int(receipt.get(field), f"build_receipt.{field}") != manifest.get(field):
