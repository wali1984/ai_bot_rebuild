...
def _iso_from_ms(value: int | None) -> str | None:
    if value is None:
        return None
    return datetime.fromtimestamp(value / 1000.0, timezone.utc).isoformat(timespec="milliseconds").replace("+00:00", "Z")


def _source_time_ms(payload: Mapping[str, Any]) -> int | None:
    for key in (
        "available_at",
        "generated_at",
        "generated_utc",
...
def _source_availability(
    *,
...
) -> tuple[dict[str, dict[str, Any]], list[str], list[str], int | None]:
    ...
    for name in REQUIRED_SOURCES:
        ...
        source_ms = _source_time_ms(payload)
        ...
        if not is_present:
            ...
            latest_ms = source_ms if latest_ms is None else max(latest_ms, source_ms)
        availability[name] = {
    ...
    return availability, missing, stale, latest_ms


def _level_component(levels: Mapping[str, Any]) -> float | None:
    explicit = _float(_first_present(levels.get("liquidation_cascade_risk"), levels.get("cascade_risk")))
    distances = [
        _float(
            _first_present(
...
def _event_component(event: Mapping[str, Any]) -> float | None:
    explicit = _float(_first_present(event.get("cascade_risk"), event.get("liquidation_cascade_risk")))
...
def _oi_component(oi: Mapping[str, Any]) -> float | None:
    change = _float(_first_present(oi.get("oi_change_pct"), oi.get("open_interest_change_pct"), oi.get("change_pct")))
    return None if change is None else _clamp(abs(change) / 0.05)


def _funding_component(funding: Mapping[str, Any]) -> float | None:
    value = _float(_first_present(funding.get("funding_skew"), funding.get("funding_rate"), funding.get("last_funding_rate"), funding.get("funding_bps")))
    if value is None:
        return None
    if abs(value) > 1.0:
...
def _long_short_component(long_short: Mapping[str, Any]) -> float | None:
    ratio = _float(_first_present(long_short.get("long_short_ratio"), long_short.get("longShortRatio"), long_short.get("ratio")))
    if ratio is None:
        long_pct = _float(_first_present(long_short.get("long_account_pct"), long_short.get("long_pct")))
        short_pct = _float(_first_present(long_short.get("short_account_pct"), long_short.get("short_pct")))
        if long_pct is not None and short_pct not in (None, 0.0):
...
def _orderbook_component(orderbook: Mapping[str, Any]) -> float | None:
    depth_collapse = _float(_first_present(orderbook.get("depth_collapse_bps"), orderbook.get("depth_collapse_score")))
    imbalance = _float(_first_present(orderbook.get("orderbook_imbalance"), orderbook.get("depth_imbalance"), orderbook.get("imbalance")))
    depth_usd = _float(_first_present(orderbook.get("market_depth_usd"), orderbook.get("orderbook_depth_usd"), orderbook.get("depth_usd")))
    components = []
    if depth_collapse is not None:
        components.append(_clamp(depth_collapse if depth_collapse <= 1 else depth_collapse / 3500.0))
...
def _spread_component(spread: Mapping[str, Any]) -> float | None:
    spread_bps = _float(_first_present(spread.get("spread_bps"), spread.get("bid_ask_spread_bps"), spread.get("real_spread_bps")))
    return None if spread_bps is None else _clamp(spread_bps / 20.0)


def _trade_tape_component(tape: Mapping[str, Any]) -> float | None:
    accel = _float(_first_present(tape.get("trade_tape_acceleration"), tape.get("aggressive_flow_acceleration"), tape.get("trade_update_rate")))
    imbalance = _float(_first_present(tape.get("trade_imbalance"), tape.get("order_flow_imbalance"), tape.get("aggressive_flow")))
    components = []
...
def _cross_asset_component(cross_asset: Mapping[str, Any]) -> float | None:
    move = _float(_first_present(cross_asset.get("correlated_move_score"), cross_asset.get("btc_eth_sol_move_score"), cross_asset.get("market_wide_risk")))
    if move is None:
...
def build_cascade_context(
    *,
    ...
    decision_iso = _iso_from_ms(parse_time_ms(decision_time)) or iso_now()
    availability, missing, stale, latest_ms = _source_availability(sources=sources, decision_time=decision_iso)
    usable = {
    ...
    proxy_values = [components[name] for name in proxy_names if components[name] is not None]
    ...
    return {
        ...
        **{key: (round(value, 8) if value is not None else None) for key, value in components.items()},
        ...
        "available_at": _iso_from_ms(latest_ms) or decision_iso,
...
def context_allows_short_trend_paper_entry(context: Mapping[str, Any], *, threshold: float = 0.30) -> tuple[bool, str | None]:
    ...
    if status in {
        ...
        if score >= threshold:
            return True, None
        return False, f"REGIME_GATE_INSUFFICIENT_CASCADE_RISK:{score:.4f}<{threshold:.2f}:cascade_context:{status}"
