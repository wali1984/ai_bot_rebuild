...
CONFIDENCE_CALIBRATION_SCHEMA_VERSION = "v2_profitability_confidence_calibration_v2"
...
def _nll(raw_probs: Sequence[float], outcomes: Sequence[int], temperature: float) -> float:
    ...
    for raw, outcome in zip(raw_probs, outcomes, strict=True):
        probability = _clamp_prob(_temperature_scaled(raw, temperature))
        total += -(
...
def brier_score(
    ...
    if len(raw_probs) != len(outcomes) or not raw_probs:
        return None
    errors = [
        (_temperature_scaled(raw, temperature) - int(outcome)) ** 2
        for raw, outcome in zip(raw_probs, outcomes, strict=True)
    ]
...
def expected_calibration_error(
    ...
    """Binned absolute calibration error after temperature scaling."""
    if len(raw_probs) != len(outcomes) or not raw_probs or bins <= 0:
        return 0.0
    buckets: list[list[tuple[float, int]]] = [[] for _ in range(bins)]
    for raw, outcome in zip(raw_probs, outcomes, strict=True):
        probability = _temperature_scaled(float(raw), temperature)
    ...
    for bucket in buckets:
        ...
        ece += (len(bucket) / len(raw_probs)) * abs(confidence - accuracy)
    return ece
...
def normalize_calibration_state(state: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    if any(value is None for value in parsed_action_counts.values()):
        ...
        action_counts = {
            action: int(parsed_action_counts[action])
            for action in CONFIDENCE_HEAD_ACTIONS
        }
...
def fit_temperature(
    ...
    *,
    ...
    lo_value = _finite_float(lo)
    hi_value = _finite_float(hi)
    if lo_value is None or hi_value is None or lo_value <= 0.0 or hi_value <= lo_value:
        return unfitted_calibration_state("CALIBRATION_SEARCH_BOUNDS_INVALID")

    grid = [lo_value + (hi_value - lo_value) * index / 40.0 for index in range(41)]
    best_temperature = min(grid, key=lambda value: _nll(probabilities, labels, value))
    radius = (hi_value - lo_value) / 40.0
    lower = max(lo_value, best_temperature - radius)
    upper = min(hi_value, best_temperature + radius)
    golden_ratio = (math.sqrt(5.0) - 1.0) / 2.0
    left = upper - golden_ratio * (upper - lower)
    right = lower + golden_ratio * (upper - lower)
    for _ in range(40):
        if _nll(probabilities, labels, left) < _nll(probabilities, labels, right):
            upper = right
        else:
            lower = left
        left = upper - golden_ratio * (upper - lower)
    ...
    brier_before = brier_score(probabilities, labels, 1.0)
    brier_after = brier_score(probabilities, labels, fitted_temperature)
    state = {
        "schema_version": CONFIDENCE_CALIBRATION_SCHEMA_VERSION,
        "fitted": True,
        "reason": None,
        ...
        "nll_before": _nll(probabilities, labels, 1.0),
        "nll_after": _nll(probabilities, labels, fitted_temperature),
        "ece_before": expected_calibration_error(probabilities, labels, 1.0),
...
def calibrate_confidence(
    *,
    ...
    scaled = (
        _temperature_scaled(raw, temperature_value)
    ...
    return {
        ...
        "temperature": float(temperature_value) if fitted else None,
        ...
        "calibration_source": (
            "checkpoint_bound_train_only_temperature_probability"
            if fitted
